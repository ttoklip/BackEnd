...
@Getter
@AllArgsConstructor
public class CartWebCreate implements PostRequest {
    ...
    @NotBlank(message = "공구 최대 인원은 필수입니다.")
    public Long partyMax;

    @Schema(
            description = "게시글에 첨부할 이미지 파일. 파일 형식은 binary이며, 지원되는 이미지 형식은 JPEG, PNG 등입니다.",
            format = "binary"
    )
    public List<MultipartFile> images;

    @Schema(
            description = "게시글에 첨부할 상품관련 URL. URL은 string이며 리스트로 여러개의 url을 사용할 수 있습니다, 클릭 시 글을 자유롭게 이동할 수 있습니다.",
